...
private:
	...
	* \brief Mode when a constant is contained within the opcode and it
	*        affects directly the register at the index given in the opcode
	*/
	struct ImmediateOpMode 
	{
		UInt32 OutAddr() { return 0; }
		UInt32 Value() { return 0; }
	};
	...
	* \brief Mode when the value contained within a register is used to modify
	*        an another register
	*/
	struct InplaceOpMode
	{
	...
	* \brief Mode when the outcome of a computation involving two different registers
	*        is stored within a third register
	*/
	struct StandardOpMode
	{
		UInt32 OutAddr() { return 0; }
...
private:
	...
	struct ArithmeticInstruction<std::divides<UInt16>>
	{
		void Execute()
		{
	...
	template<class OpMode>
	struct ArithmeticInstruction<std::minus<UInt16>>
	{
		void Execute()
		{
	...
	template<class OpMode>
	struct ArithmeticInstruction<std::multiplies<UInt16>>
	{
		void Execute()
		{
	...
	template<class OpMode>
	struct ArithmeticInstruction<std::plus<UInt16>>
	{
		void Execute()
		{
